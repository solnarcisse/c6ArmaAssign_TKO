resource "aws_autoscaling_group" "Calif_asg" {
  name_prefix           = "Calif-auto-scaling-group-"
  min_size              = 2
  max_size              = 8
  desired_capacity      = 4
  vpc_zone_identifier   = [
  ...
  force_delete               = true
  target_group_arns          = [aws_lb_target_group.Calif_tg.arn]

  launch_template {
    id      = aws_launch_template.Calif_LT.id
    version = "$Latest"
  }
